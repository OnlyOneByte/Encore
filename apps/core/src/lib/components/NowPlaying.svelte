<script lang="ts">
	...
	let {
		...
		keyShiftEnabled = false, onplaypause, onprev, onnext
	}: Props = $props();
</script>
...
<div class="now">
	<div class="inner">
		...
		<div class="key" class:disabled={!keyShiftEnabled}>
			<span class="lab">Key</span>
			<button disabled={!keyShiftEnabled}>− semitone</button>
			<button disabled={!keyShiftEnabled}>＋ semitone</button>
		</div>
	</div>
</div>
...
<style>
	...
	.key button { flex: 1; border: 1px dashed #3a3a55; background: transparent; color: #6b6b8c; border-radius: 10px; padding: 6px; font-size: 0.8rem; }
</style>
